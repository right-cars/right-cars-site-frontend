...
import MainInfo from "@/views/Auction/MainInfo/MainInfo";

import { getCarById} from "@/api/auctions";
...
export default async function CarPage({
  ...
  const {car, ...auction} = await getCarById(id);

  return (
    <>
       {/* @ts-expect-error */}
      <MainInfo data={car} auction={auction} pageId={id} />
      {/* <SimpleCardList
        data={cardListData}
        title="how it works"
        minHeight={284}
